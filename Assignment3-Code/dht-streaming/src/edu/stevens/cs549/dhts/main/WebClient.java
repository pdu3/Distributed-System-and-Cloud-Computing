...
public class WebClient {
	...
	public String[] get(NodeInfo n, String k) throws DHTBase.Failed {
		UriBuilder ub = UriBuilder.fromUri(n.addr).path("get");
		URI getPath = ub.queryParam("key", k).build();
		info("client get(" + getPath + ")");
		Response response = getRequest(getPath);
		if (response == null || response.getStatus() >= 300) {
			throw new DHTBase.Failed("GET /get?id=ID");
		} else {
	...
	public EventSource listenForBindings(NodeInfo node, int id, String skey) throws DHTBase.Failed {
		...
		URI uri = UriBuilder.fromUri(node.addr).path("listen").queryParam("id",id).queryParam("key", skey).build();
		info("Server Side listenForBindings(" + uri + ")");
		WebTarget target = listenClient.target(node.addr).path("listen").queryParam("id", id).queryParam("key", skey);
	...
	public void listenOff(NodeInfo node, int id, String skey) throws DHTBase.Failed {
		// TODO listen for SSE subscription requests on http://.../dht/listen?key=<key>
		// On the service side, don't expect LT request or response headers for this request.
		URI listenOffURI = UriBuilder.fromUri(node.addr).path("listen").queryParam("id",id).queryParam("key", skey).build();
        info("Server Side listenOff(" + listenOffURI + ")");
        Response response = delRequest(listenOffURI);
        if(response == null || response.getStatus() >= 300) {
